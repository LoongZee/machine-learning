...
y = tf.placeholder(tf.int64, [None, 227, 227, 1])
keep_prob = tf.placeholder(tf.float32)
# Initialize model
model = fcn_alex(x, keep_prob, num_classes, train_layers)
...
with tf.Session() as sess:
    ...
    train_batch, train_label_batch = input_tfrecords.get_train_batch("train.tfrecords", batch_size, imagewh = 227)
    test_batch, test_label_batch = input_tfrecords.get_test_data("test.tfrecords", 10, imagewh = 227)
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 1 == 0:
            ...
            for i, img, label, pre in zip(range(X_data.shape[0]), X_data, y_data, _out):
                label = np.concatenate([label * 255, np.zeros([227, 227, 2])], 2)
                pre = np.concatenate([np.zeros([227, 227, 1]), np.expand_dims(pre * 255, 2), np.zeros([227, 227, 1])], 2)
