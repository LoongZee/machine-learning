...
filewriter_path = "finetune_net/filewriter"
checkpoint_path = "finetune_net/checkpoint"
...
model = fcn_net(x, keep_prob, num_classes, is_train)
    ...
    for epoch in range(num_epochs):
        ...
        while step <= train_batches_per_epoch:
            ...
            _train_op, _loss, _acc, _s = sess.run([optimizer, loss, accuracy, model_summary],
                                                  feed_dict={x: X_data, y: y_data, keep_prob: dropout_rate, is_train:True})
        ...
        if (epoch + 1) % display_step == 0:
            ...
            _loss, _acc, _out, _s = sess.run([loss, accuracy, output, model_summary],
                                             feed_dict={x: X_data, y: y_data, keep_prob: 1., is_train:False})
            ...
            for i, img, label, pre in zip(range(X_data.shape[0]), X_data, y_data, _out):
                ...
                utils.save_image(img, 'output/img', str(epoch) + '_' + str(i + 1))
                utils.save_image(label, 'output/label', str(epoch) + '_' + str(i + 1))
                utils.save_image(pre, 'output/pre', str(epoch) + '_' + str(i + 1))
